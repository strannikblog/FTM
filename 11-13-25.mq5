...
int OnInit()
{
   ...
   for(int i = 0; i < PositionsTotal(); i++)
   {
      if(PositionGetSymbol(i) == _Symbol)
      {
         ...
         double exitPct2 = (g_NumberOfLevels >= 2) ? g_ExitPercent2 : 0;

         // Calculate TP1 lots
         if(g_NumberOfLevels == 1)
         {
...
void ExecuteBuyOrder()
{
   ...
   if(result)
   {
      ...
      // Only create Active SL line if EA is managing SL (not broker)
      if(inpAutoExecuteSL)
      {
...
void LoadSettingsFromFile(bool loadPercentages = true)
{
   ...
   while(!FileIsEnding(fileHandle))
   {
      ...
      if(equalPos > 0 && section == "GlobalVariables")
      {
         ...
         else if(loadPercentages && key == "NumberOfLevels")
            g_NumberOfLevels = (int)StringToInteger(value);
...
void SaveSettingsToFile()
{
   ...
   FileWriteString(fileHandle, "NumberOfLevels=" + IntegerToString(g_NumberOfLevels) + "\n");
