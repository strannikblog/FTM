...
#property description "3-Level Dynamic Risk Management with Static Override - Rounding Fix"
...
struct RiskManagerState {
    ...
    double currentRiskPercent;    // Current active risk (final capped value)
    ...
    double dynamicRiskPercent;     // Risk before static override applied (from dynamic system)
...
void ApplyStaticOverride(RiskManagerState &state) {
    if(!inpEnableStaticOverride) {
        ...
        state.currentRiskPercent = state.dynamicRiskPercent; // Pass through dynamic risk
        return;
    }
    ...
    // Apply override: Final risk = MIN(dynamic risk, static ceiling)
    if(staticCap == 0.0) {
        ...
        // BUG FIX: Use dynamicRiskPercent (fresh from dynamic system), not currentRiskPercent (stale value)
        double finalRisk = MathMin(state.dynamicRiskPercent, staticCap);

        if(finalRisk < state.dynamicRiskPercent) {
            Print("⚠️ STATIC OVERRIDE: Dynamic risk ", DoubleToString(state.dynamicRiskPercent, 2),
                  "% capped at ", DoubleToString(staticCap, 2), "% due to ",
                  DoubleToString(state.currentDrawdown, 2), "% drawdown");
        }
...
void UpdateRiskFile(double currentRiskPercent) {
    ...
    if(fileHandle != INVALID_HANDLE) {
        ...
        Print("🔄 RISK % LOGGED: ", DoubleToString(currentRiskPercent, 2), "% has been written to file for Trade Manager");
...
string StateToCsv(const RiskManagerState &state) {
    // Updated to 26 fields (added tradingHalted)
    return StringFormat(
        "%.6f,%.6f,%.6f,%.6f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.6f,%.6f,%.6f,%d,%d,%d,%d,%d,%s,%llu,%s,%d",
        ...
        (int)state.tradingHalted,      // Added field
...
bool CsvToState(const string csvStr, RiskManagerState &state) {
    ...
    // Support both old format (25 fields) and new format (26 fields)
    if(count != 25 && count != 26) {
        Print("❌ Invalid state file format. Expected 25 or 26 fields, got ", count);
        return false;
    }
    ...
    state.currentDrawdown = StringToDouble(parts[12]);
    state.staticRiskCeiling = StringToDouble(parts[13]);
    state.dynamicRiskPercent = StringToDouble(parts[14]);

    if(count == 26) {
        // New format with tradingHalted field
        state.tradingHalted = (bool)StringToInteger(parts[15]);
        state.monthStart = (datetime)StringToInteger(parts[16]);
        state.weekStart = (datetime)StringToInteger(parts[17]);
        state.dayStart = (datetime)StringToInteger(parts[18]);
        state.consecutiveLosses = (int)StringToInteger(parts[19]);
        state.lastTradeType = parts[20];
        state.lastProcessedTicket = StringToInteger(parts[21]);
        state.accountNumber = parts[22];
        state.lastUpdateTime = (datetime)StringToInteger(parts[23]);
    } else {
        // Old format (25 fields) - will be recalculated
        state.tradingHalted = false;
...
void InitializeState(RiskManagerState &state) {
    ...
    // Start at maximum risk level (dynamic system)
    state.dynamicRiskPercent = inpMaxRiskPercent;
    state.currentRiskPercent = inpMaxRiskPercent;  // Will be set by ApplyStaticOverride
...
void CreateDisplay() {
    ...
    int panelWidth = 245;  // Panel width
    int textPadding = 10;
    ...
    if(inpShowDrawdown && inpEnableStaticOverride) {
        panelHeight = 220; // Height with drawdown + static override line
        labelCount = 11;
    } else if(inpShowDrawdown || inpEnableStaticOverride) {
        panelHeight = 205; // Height with one additional line
        labelCount = 10;
    } else {
        panelHeight = 185; // Height without additional lines
        labelCount = 9;
    }
...
void UpdateDisplay() {
    ...
    ObjectSetString(0, g_labelName + "_Title", OBJPROP_TEXT, "🛡️ RISK MANAGER v1.58");

    // BUG FIX: Show currentRiskPercent (final capped value), not dynamicRiskPercent
    string levelText = "Level: " + currentLevel + " (" + DoubleToString(g_state.currentRiskPercent, 2) + "%)";
...
int OnInit() {
    ...
    Print("🚀 Risk Manager Indicator v1.58 (Rounding Fix) starting on ", _Symbol);
    ...
    g_initialized = true;
    Print("✅ Risk Manager Indicator v1.58 initialized with Rounding Fix");
